...
(def cc-nr (clojure.set/map-invert nr-cc))
...
(def ^:const country-code--country
  (conj
   #_(country-code--country-nv-i18n)
   (into {} (map (fn [[k v]] {(get cc-nr k) v}) (country-code--country-nv-i18n)))
...
(def ^:const country--country-code
  ...
  (country--country-code \"Germany\") ;; => \"DE\"
...
(defn get-country-name
  "Country name from 2-letter country code:
  (get-country-name\"DE\") ;; => \"Germany\"
  "
  [ccode]
...
(defn country-alias
  ...
  [ccode-nr]
  (let [ccode (get nr-cc ccode-nr)
        up-ccode (cstr/upper-case ccode)
...
(defn country-name-aliased
  ...
  (country-name-aliased \"FR\") ;; => \"France\"
  "
  [ccode]
  (if (in?
       [
