...
#_(defn country-names []
  (->> regions
       (mapv (fn [[country & rest]] country))))

#_(defn continent-countries []
  (->> regions
       (group-by (fn [[_ continent & rest]] continent))
       (map (fn [[k v]]
              {k (->> v
                      (mapv (fn [[country & rest]] country)))}))
       (reduce into {})))
...
(def continent-countries-map
  ...
  [
   ...
   ["AF" "SD" "SDN" "729" "Sudan, Republic of"]
   ...
   ["EU" "MK" "MKD" "807" "Macedonia, The Former Yugoslav Republic of"]
   ...
   ["AS" "XD"  nil   nil  "United Nations Neutral Zone"]
   ["AS" "XE"  nil   nil  "Iraq-Saudi Arabia Neutral Zone"]
   ["AS" "XS"  nil   nil  "Spratly Islands"]
   ["OC" "XX"  nil   nil  "Disputed Territory"]
   ])

(defn country-codes-of-continent [continent-code]
  (->> continent-countries-map
       (filter (fn [[continent-c country-c & rest]]
                 (= continent-code continent-c)))
       (map (fn [[_ country-c & rest]] country-c))
       (into #{})))
